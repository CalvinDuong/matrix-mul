...
int convolve(matrix_t *a_matrix, matrix_t *b_matrix, matrix_t **output_matrix) {
  ...
  // output_matrix
    ...
    *output_matrix = (matrix_t*)malloc(sizeof(matrix_t));
    ...
    //register int jump = a_matrix -> cols - (*output_matrix) -> cols;
    //#pragma omp parallel
    //{
    //register int j = 0;
    //register int k =0;
    
    /*for(register int i  = 0; i < outrows*outcols ; i++){
        ...
        if (k == outcols){
            ...
            j = j + jump;
                ...
                //#pragma omp for
        for (register int x = j ; x <j + (brows) * (acols); x += (acols)){ //top left of a subpart
            register __m256i mul = _mm256_setzero_si256();
            //int sum = 0;
            register int y;
            for(y=0;y<bcols/16*16;y+=16) {
                register __m256i a = _mm256_loadu_si256((__m256i *) ((am)+x+y));
                register __m256i b = _mm256_loadu_si256((__m256i *) ((fbd) +counter));
                register __m256i m = _mm256_mullo_epi32(a,b);
                mul = _mm256_add_epi32(mul,m);
                a = _mm256_loadu_si256((__m256i *) (am+x+y+8));
                b = _mm256_loadu_si256((__m256i *) ((fbd) +counter+8));
                m = _mm256_mullo_epi32(a,b);
                mul = _mm256_add_epi32(mul,m);
                counter +=16;
            }
            ...
            _mm256_storeu_si256((__m256i *) tmp_arr,mul);
            for(;y<bcols;y++) {
                int ab =   am[x+y] * fbd[counter];
                tmp_arr[0] = tmp_arr[0] + ab;
                counter +=1;
            }
            //#pragma omp critical
            sum+=  tmp_arr[0] + tmp_arr[1] + tmp_arr[2] + tmp_arr[3] + tmp_arr[4] + tmp_arr[5] + tmp_arr[6] + tmp_arr[7];
